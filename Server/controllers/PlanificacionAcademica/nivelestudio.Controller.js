...
export const updateNivelEstudio = async (req, res) => {
    try {
        const { idnivelEstudio } = req.params; // El id se pasa como parámetro en la URL
        ...
        const [exists] = await db.query("SELECT 1 FROM nivelestudio WHERE idnivelEstudio = ?", [idnivelEstudio]);
        if (!exists.length) return res.status(404).json({ message: "El nivelestudio no existe" });
        // Actualiza los datos del nivelestudio
        const [result] = await db.query("UPDATE nivelestudio SET nombre = ?, descripcion = ?, sigla = ? WHERE idnivelEstudio = ?", [nombre,descripcion, sigla, idnivelEstudio]);
...
export const deleteNivelEstudio = async(req, res) => {
    try {
      const {idnivelEstudio } = req.params;
      const [nivelestudio] = await db.query("SELECT nombre FROM nivelestudio WHERE idnivelEstudio = ?", [idnivelEstudio]);
      if (!nivelestudio.length) return res.status(404).json({ message: "Nivel Estudio no encontrado" });
      const [rows] = await db.query("DELETE FROM nivelestudio WHERE idnivelEstudio = ?", [idnivelEstudio]);
      rows.affectedRows
